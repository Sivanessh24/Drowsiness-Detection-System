#python drowniness.py --webcam webcam_index
from imutils.video import VideoStream#to veiw the video from the 
from imutils import face_utils#12456789
...
def sound():
#    playsound(r"F:\Shyam\1.python project\my proj\drowssiness detection\alarm.wav")
...
def alarm():
    global alarm_status,countm,count1
    count=0
    while alarm_status:
        if count>=3 :
            sound()
...
while True:
    ...
    for rect in rects:
        ...
        cv2.putText(frame, 'EAR:'+str(ear), (225, 325),
        ...
        if ear < E_A_R:#ear<0.3
            ...
            if COUNTER >= EYE_FPS:#COUNTER>=72/24
                ...
                if alarm_status == False:
                    alarm_status = True
                    t=Thread(target=alarm)#only function name is enough
